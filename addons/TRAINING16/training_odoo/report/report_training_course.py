...
class CourseXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        row = 5
        sheet.freeze_panes(6, 10)  # Do bekuin atau tidak bisa di scroll ke kanan 6 - 10
        ...
        for x in obj.session_line:
            ...
            partner.append(
                x.partner_id.name if x.partner_id and x.partner_id.name else ""
            )
            start_date.append(
                x.start_date.strftime("%d-%m-%Y") if x.start_date else ""
            )  # menggunakan If Agar ketika data kosong tidak error dan tidak dimasukan
